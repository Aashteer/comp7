    ...
    def __init__(self):
        ...
        self.data = {
            'ru': {
                ...
                'Открыть пример if-else (КР)': 'Открыть пример if-else (КР)',
                'Открыть пример max.c': 'Открыть пример max.c',
                'AST не построено: исправьте синтаксические ошибки.': 'AST не построено: исправьте синтаксические ошибки.',
                'Не удалось построить AST.': 'Не удалось построить AST.',
                'КР: max.c — AST, IR, CFG, cmov': 'КР: max.c — AST, IR, CFG, cmov',
                'КР: AST и оптимизации готовы': 'КР: AST и оптимизации готовы',
                'Для C откройте пример max.c (условные переходы).': 'Для C откройте пример max.c (условные переходы).',
                'Режим C (max): лексер Python не применяется.\n': 'Режим C (max): лексер Python не применяется.\n',
                'Запуск собственного AST/IR/CFG (F6)...': 'Запуск собственного AST/IR/CFG (F6)...',
                'Синтаксических ошибок не обнаружено; доп. задание выполнено': 'Синтаксических ошибок не обнаружено; доп. задание выполнено',
                'Не найден файл:': 'Не найден файл:',
            },
            'en': {
                ...
                'Открыть пример if-else (KР)': 'Open if-else example (KR)',
                'Открыть пример if-else (КР)': 'Open if-else example (KR)',
                'Открыть пример max.c': 'Open max.c example',
                'AST не построено: исправьте синтаксические ошибки.': 'AST not built: fix syntax errors.',
                'Не удалось построить AST.': 'Failed to build AST.',
                'КР: max.c — AST, IR, CFG, cmov': 'KR: max.c — AST, IR, CFG, cmov',
                'КР: AST и оптимизации готовы': 'KR: AST and optimizations ready',
                'Для C откройте пример max.c (условные переходы).': 'For C, open max.c example (conditional jumps).',
                'Режим C (max): лексер Python не применяется.\n': 'C mode (max): Python lexer not used.\n',
                'Запуск собственного AST/IR/CFG (F6)...': 'Running custom AST/IR/CFG (F6)...',
                'Синтаксических ошибок не обнаружено; доп. задание выполнено': 'No syntax errors; bonus task completed',
