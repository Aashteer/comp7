...
from if_ast import IfASTBuilder, format_ast
from if_ir import ast_to_tac, run_if_optimizations, format_if_bonus_report, tac_to_canonical
from c_max_ir import is_c_max_program, is_c_source, build_max_report_from_source
...
class TextEditor(QMainWindow):
    ...
    def initUI(self):
        ...
        self.bonus_tab = QWidget()
        bonus_layout = QVBoxLayout(self.bonus_tab)
        self.bonus_text = QTextEdit()
        self.bonus_text.setReadOnly(True)
        self.bonus_text.setStyleSheet(
            "QTextEdit { background-color: #1e1e1e; color: #ffffff; "
            "border: 1px solid #555555; font-family: Consolas, monospace; }"
        )
        bonus_layout.addWidget(self.bonus_text)
        ...
        self.result_tabs.addTab(self.bonus_tab, self.tr("КР: AST / TAC"))
    ...
    def create_menu(self):
        ...
        bonus_act = QAction(self.tr('Доп. задание: AST и TAC'), self)
        bonus_act.setShortcut('F6')
        bonus_act.triggered.connect(self.run_bonus_analysis)
        run_menu.addAction(bonus_act)

        open_if_act = QAction(self.tr('Открыть пример if-else (КР)'), self)
        open_if_act.triggered.connect(self.open_if_example)
        run_menu.addAction(open_if_act)

        open_max_act = QAction(self.tr('Открыть пример max.c'), self)
        open_max_act.triggered.connect(self.open_max_example)
        run_menu.addAction(open_max_act)
    ...
    def retranslateUi(self):
        ...
        self.result_tabs.setTabText(4, self.tr("КР: AST / TAC"))
    ...
    def open_if_example(self):
        path = self._project_root() / "examples" / "if_else_demo.txt"
        if path.is_file():
            self.open_file_with_path(str(path))
        else:
            QMessageBox.warning(self, self.tr('Ошибка'), f"{self.tr('Не найден файл:')} {path}")

    def open_max_example(self):
        path = self._project_root() / "examples" / "max.c"
        if path.is_file():
            self.open_file_with_path(str(path))
        else:
            QMessageBox.warning(self, self.tr('Ошибка'), f"{self.tr('Не найден файл:')} {path}")
    ...
    def open_file(self):
        file_name, _ = QFileDialog.getOpenFileName(
            ...
            'C/C++ (*.c *.cpp *.h);;Текстовые (*.txt);;Все файлы (*)'
        )
        if file_name:
    ...
    def run_bonus_analysis(self):
        ...
        text = tab.get_text()
        filename = getattr(tab, 'current_file', None)
        self.bonus_text.clear()

        if is_c_source(text, filename) and is_c_max_program(text):
            report = build_max_report_from_source(text)
            self.bonus_text.setPlainText(report)
            self.result_tabs.setCurrentWidget(self.bonus_tab)
            self.status_bar.showMessage(self.tr('КР: max.c — AST, IR, CFG, cmov'), 5000)
            return

        if is_c_source(text, filename):
            self.bonus_text.setPlainText(
                self.tr('Для C откройте пример max.c (условные переходы).')
            )
            self.result_tabs.setCurrentWidget(self.bonus_tab)
            return

        results = self.scanner.analyze(text)
        parse_result = Parser(results['tokens'], lang=self.translator.lang).parse()
        if not parse_result.ok:
            self.bonus_text.setPlainText(
                self.tr('AST не построено: исправьте синтаксические ошибки.')
            )
            self.result_tabs.setCurrentWidget(self.bonus_tab)
            return

        ast_node = IfASTBuilder(results['tokens']).build()
        if not ast_node:
            self.bonus_text.setPlainText(self.tr('Не удалось построить AST.'))
            self.result_tabs.setCurrentWidget(self.bonus_tab)
            return

        instrs = ast_to_tac(ast_node)
        opts = run_if_optimizations(instrs)
        report = format_if_bonus_report(
            format_ast(ast_node),
            instrs,
            opts,
            tac_to_canonical(ast_node),
        )
        self.bonus_text.setPlainText(report)
        self.result_tabs.setCurrentWidget(self.bonus_tab)
        self.status_bar.showMessage(self.tr('КР: AST и оптимизации готовы'), 5000)
    ...
    def start_analyzer(self):
            ...
            if is_c_source(text, filename) and is_c_max_program(text):
                self.result_text.setPlainText(
                    self.tr('Режим C (max): лексер Python не применяется.\n')
                    + self.tr('Запуск собственного AST/IR/CFG (F6)...')
                )
            ...
            if parse_result.ok:
                ...
                self.status_bar.showMessage(
                    ...
                    f"{self.tr('Синтаксических ошибок не обнаружено; доп. задание выполнено')}"
                )
            else:
                self.status_bar.showMessage(
                    f"{self.tr('Анализ завершен')}. {self.tr('Лексем:')} {len(results['tokens'])}, "
    ...
    def show_help(self):
        help_text = self.tr('Справка по текстовому редактору') + '\n\n' + \
                    ...
                    "- " + self.tr('Доп. задание: AST и TAC') + ": F6\n\n" + \
